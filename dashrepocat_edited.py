...
from adaptation import basic_dash, basic_dash2, weighted_dash, netflix_dash, dash_bola, empirical_dash, retransmission
...
def get_mpd(url):
    ...
    quic_cmd = "ndncatchunks --fast-conv --init-ssthresh 12 -qTD /edu/umass/" + url
    log = open(url, "wb")
    config_dash.LOG.info(quic_cmd)
    p = subprocess.call(shlex.split(quic_cmd), shell=False, stdin=None, stdout=log, stderr=log)
    config_dash.LOG.info("MPD DOWNLOAD PROCESS STATUS: " + str(p))
...
# noinspection DuplicatedCode
def download_segment_squad(segment_url, dash_folder):
    """ Module to download the segment """
    abandonReq = False
    parsed_uri = urlparse.urlparse(segment_url)
    segment_path = '{uri.path}'.format(uri=parsed_uri)
...
def start_playback_smart(dp_object, domain, playback_type=None, download=False, video_segment_duration=None,
    ...
    """ Module that downloads the MPD-FIle and download
        ...
        :param playback_type:   The type of playback
                                1. 'BASIC' - The basic adaptation scheme
    ...
    dash_player = dash_buffer.DashPlayer((dp_object.playback_duration - (segment_number * video_segment_duration)),
    ...
    while segment_number < len(dp_list):
        ...
        if SEGMENT_LIMIT:
            ...
            if segment_number > int(SEGMENT_LIMIT):
                ...
                break
        if segment_number == dp_object.video[current_bitrate].start:
            current_bitrate = bitrates[0]
        else:
            if playback_type.upper() == "BASIC":
                ...
                if segment_number < len(dp_list) - 1 + dp_object.video[current_bitrate].start:
                    try:
                ...
                if segment_number < len(dp_list) - 1 + dp_object.video[bitrate].start:
                    try:
                        ...
                        if not bola_init_state:
                            bolaObj, delay = dash_bola.calculateInitialState(segment_download_rate, current_bitrate,
                                                                             bolaObj)
                            bola_init_state = True
                        bolaObj, delay = dash_bola.bola_dash(dash_player.buffer.__len__(), segment_download_rate,
                                                             current_bitrate, last_requests_start, last_requests_finish,
                                                             last_requests_tput, bolaObj)
                        ...
                        if not os.path.exists(bola_buffer_log_file):
                            ...
                            stats = (timeit.default_timer() - start_dload_time, str(dash_player.buffer.__len__()),
                                     current_bitrate, (segment_download_rate / 8.0), bolaObj.state)
                        else:
                            header_row = None
                            stats = (timeit.default_timer() - start_dload_time, str(dash_player.buffer.__len__()),
                                     current_bitrate, (segment_download_rate / 8.0), bolaObj.state)
            ...
            elif playback_type.upper() == "SQUAD":
                ...
                current_bitrate, decision_rate = empirical_dash.empirical_dash(average_segment_sizes, segment_number,
                                                                               bitrates, segment_download_time,
                                                                               current_bitrate,
                                                                               ...
                                                                               bitrate_history, segment_w_chunks,
                                                                               DOWNLOAD_CHUNK)  # MZ
                    ...
                    if current_bitrate <= bitrate_history[-1] or dash_player.buffer.__len__() < buffer_lower:
                        ...
                        if dash_player.buffer.__len__() >= buffer_upper:  # MZ
                            ...
                            if bitrate_holder == 1:
                                ...
                                bitrate_holder = 1
                            elif (current_bitrate >= bitrates[int(bitrates.index(bitrate_history[-1]) - 2)]) and (
                                    current_bitrate < bitrate_history[-1]):
                                current_bitrate = bitrate_history[-1]
                ...
                if segment_number > 10 and retrans:
                    ...
                    if dash_player.buffer.__len__() >= (
                            RETRANS_THRESHOLD_UPPER * config_dash.NETFLIX_BUFFER_SIZE) or RETRANSMISSION_SWITCH:
                ...
                with open('empirical-dash-chosen-rate.txt', 'a') as emp:
                    emp.write(str(segment_number) + '\t' + str(bitrate_actual_time) + '\t' + str(
                        segment_download_rate * 8) + '\t' + str(decision_rate) + '\t' + str(
                        current_bitrate) + '\t' + str(dash_player.buffer.__len__()) + '\n')
                if dash_player.buffer.__len__() >= config_dash.NETFLIX_BUFFER_SIZE:  # MZ
                    delay = 1
                else:
                    delay = 0
                if not RETRANS_OFFSET:
                    bitrate_history.append(current_bitrate)
                print "-------------+++++++++++++"
                print segment_number
                print dp_list[segment_number][current_bitrate]
                #parsed_uri = urlparse.urlparse(segment_url)
                #segment_path = '{uri.path}'.format(uri=parsed_uri)
                #print urlparse.urljoin(domain, segment_url)
                print "-------------+++++++++++++"
        ...
        if delay:
            ...
            try:
                config_dash.LOG.info(
                    "{}: Started downloading segment {} with segment number {} and DP_LIST length {}".format(
                        playback_type.upper(), segment_url, segment_number, len(dp_list)))
                if playback_type.upper() == "BOLA":
                    if segment_number < len(dp_list) - 1:
                        bola_seg_size = get_segment_sizes(dp_object, segment_number)
                        start_time = timeit.default_timer()
                        segment_size, segment_filename, bolaObj = download_segment_bola(segment_url, file_identifier,
                                                                                        bolaObj)
                    ...
                    start_time = timeit.default_timer()
                    segment_size, segment_filename = download_segment_squad(segment_url, file_identifier)
                    finish_time = timeit.default_timer()
        ...
        config_dash.LOG.info(
            "{} : The total downloaded = {}, segment_size = {}, segment_number = {} segment_download_time = {}".format(
                playback_type.upper(),
...
def get_average_segment_sizes(dp_object):
    """
    Module to get the average segment sizes for each bitrate
    :param dp_object:
    :return: A dictionary of average segment sizes for each bitrate
    """
    ...
    config_dash.LOG.info("The average segment size for is {}".format(average_segment_sizes.items()))
...
def start_playback_all(dp_object, domain):
    ...
    #     # of the download of the session
...
def main():
    ...
    elif "squad" in PLAYBACK.lower():
        config_dash.LOG.critical("Started Hello-DASH Playback")
        start_playback_smart(dp_object, domain, "SQUAD", DOWNLOAD, video_segment_duration, RETRANS)
