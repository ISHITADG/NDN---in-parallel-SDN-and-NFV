...
def get_mpd(url):
    ...
    print url
    quic_cmd="../../../ndnperf/c++/client/ndnperf -p ndn:/edu/umass -d "+url+" -w 16"
...
def download_segment(segment_url, dash_folder):
    ...
    quic_cmd="../../../ndnperf/c++/client/ndnperf -p ndn:/edu/umass -d "+segment_url+" -w 16"
    ...
    for segment_number, segment in enumerate(dp_list, dp_object.video[current_bitrate].start):
        ...
        if segment_number == dp_object.video[bitrate].start:
            ...
            if playback_type.upper() == "BASIC":
                ...
                if dash_player.buffer.__len__() > config_dash.BASIC_THRESHOLD:
                    delay = dash_player.buffer.__len__() - config_dash.BASIC_THRESHOLD
